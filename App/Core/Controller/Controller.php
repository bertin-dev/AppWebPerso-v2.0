...
class Controller
{
    ...
    public static function affiche_menu($idpage) {
        ...
        if($resultat == 0) {
            ...
            $menu_retour .= '<li class="dropdown"> <a href="Portfolio">Accueil</a></li> ';
            ...
            WHERE id_page<8 AND ref_id_parent=' . $idpage, get_called_class()) as $retour):
                ...
                $menu_retour .= 'href="Portfolio?id_page='.$retour->id_page.'" title="'.utf8_encode($retour->titre) .'">';
                //$menu_retour .= '<a href="Portfolio?id_page='.$retour->id_page.'">';
               ...
               /* if($retour->id_page ==='3')
                ...
                /*if($retour->id_page ==='3')
                {
                    ...
                    $menu_retour .= '<li>';
                    $menu_retour .= '<a class="page-scroll" href="Portfolio?id_page='.$retour->id_page.'" title="'.$retour->titre.'">';
    ...
    /*lors de l'execution cette fonction magique est utilisé lorsqu'on a à faire a une variable qui n'existe pas
        ...
        {
            return 'Portfolio?page='.$this->titre.'title='.$this->titre;
        }
    ...
    /*public static function affiche_chemin_fer($idpage) {
        ...
        if ($idpage != 0) {
           ...
           foreach (\App::getDB()->query('SELECT `titre`, `ref_Id_Parent` FROM `page` WHERE `id_page` ='.$idpage) as $nbre):
            ...
            $chemin .= ' -> <a href="Portfolio?id_page='.$idpage.'">'.$titrepage.'</a>';
